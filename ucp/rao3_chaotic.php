        ...
        for ($generation = 0; $generation <= $this->maximum_generation - 1; $generation++) {
            ...
            if ($generation > 0) {
                ...
                for ($i = 0; $i <= $this->particle_size - 1; $i++) {
                    ...
                    $xSimple = $particles[$generation][$i]['xSimple'] + $r11[$generation] * ($best_particles[$generation]['xSimple'] - abs($worst_particles[$generation]['xSimple'])) + ($r12[$generation] * (abs($candidate_xSimple1) - $candidate_xSimple2));

                    $xAverage = $particles[$generation][$i]['xAverage'] + $r21[$generation] * ($best_particles[$generation]['xAverage'] - abs($worst_particles[$generation]['xAverage'])) + ($r22[$generation] * (abs($candidate_xAverage1) - $candidate_xAverage2));

                    $xComplex = $particles[$generation][$i]['xComplex'] + $r31[$generation] * ($best_particles[$generation]['xComplex'] - abs($worst_particles[$generation]['xComplex'])) + ($r32[$generation] * (abs($candidate_xComplex1) - $candidate_xComplex2));
...
for ($generation = 1; $generation <= $maximum_generation; $generation++) {
    ...
    $fp = fopen('hasil_rao3_chaotic.txt', 'a');
    fputcsv($fp, $data);
    fclose($fp);
    $temps = [];
}
