...
class EvolutionaryRaoAlgorithm
{
    ...
    function findSolution($project)
    {
        for ($generation = 0; $generation <= $this->maximum_generation - 1; $generation++) {
            ## Generate population
            if ($generation === 0) {
                $r1[$generation + 1] = $this->tent(0.8);
                $r2[$generation + 1] = $this->tent(0.8);
                $r3[$generation + 1] = $this->tent(0.8);
            ...
            if ($generation > 0) {
                $r1[$generation + 1] = $this->tent($r1[$generation]);
                $r2[$generation + 1] = $this->tent($r2[$generation]);
                $r3[$generation + 1] = $this->tent($r3[$generation]);
...
for ($generation = 1; $generation <= $maximum_generation; $generation++) {
    ...
    $fp = fopen('hasil_rao_chaotic.txt', 'a');
    fputcsv($fp, $data);
    fclose($fp);
    $temps = [];
}
