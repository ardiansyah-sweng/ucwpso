        ...
        for ($generation = 0; $generation <= $this->maximum_generation - 1; $generation++) {
            ...
            if ($generation > 0) {
                for ($i = 0; $i <= $this->particle_size - 1; $i++) {
                    ...
                    $xSimple = ($particles[$generation][$i]['xSimple'] + $r11 * ($best_particles[$generation]['xSimple'] - $worst_particles[$generation]['xSimple'])) + ($r12 * (abs($candidate_xSimple1) - abs($candidate_xSimple2)));

                    $xAverage = $particles[$generation][$i]['xAverage'] + $r21 * ($best_particles[$generation]['xAverage'] - $worst_particles[$generation]['xAverage']) + ($r22 * (abs($candidate_xAverage1) - abs($candidate_xAverage2)));

                    $xComplex = $particles[$generation][$i]['xComplex'] + $r31 * ($best_particles[$generation]['xComplex'] - $worst_particles[$generation]['xComplex']) + ($r32 * (abs($candidate_xComplex1) - abs($candidate_xComplex2)));
...
foreach ($results as $key => $result){
    ...
    $fp = fopen('hasil_rao2_estimated.txt', 'a');
    fputcsv($fp, $data);
    fclose($fp);
}
