...
class ParticleSwarmOptimizer
{
    ...
    function minimalAE($pbests)
    {
        foreach ($pbests as $val) {
            $ae[] = $val['ae'];
        }
        return $pbests[array_search(min($ae), $ae)];
    }

    function Mbest($pbests)
    {
        $ret['xSimple'] = array_sum(array_column($pbests, 'xSimple')) / $this->swarm_size;
        $ret['xAverage'] = array_sum(array_column($pbests, 'xAverage')) / $this->swarm_size;
        $ret['xComplex'] = array_sum(array_column($pbests, 'xComplex')) / $this->swarm_size;
        return $ret;
    }
    ...
    function findSolution($project)
    {
        ...
        for ($iteration = 0; $iteration <= $this->max_iteration; $iteration++) {
            ...
            if ($iteration === 0) {
                ...
                $Mbest[$iteration + 1] = $this->Mbest($Pbests[$iteration + 1]);
            } ## End Generate Population

            if ($iteration > 0) {
                for ($i = 0; $i <= $this->swarm_size - 1; $i++) {
                    $Mbest_simple = $Mbest[$iteration]['xSimple'];
                    $Mbest_average = $Mbest[$iteration]['xAverage'];
                    $Mbest_complex = $Mbest[$iteration]['xComplex'];
                    ...
                    $vSimple = $this->velocity($inertia, $R1, $R2, $vSimple, $xSimple, $SPbests_simple, $Mbest_simple);
                    if ($vSimple > $vMaxSimple) {
                        $vSimple = $vMaxSimple;
                    }
                    $vAverage = $this->velocity($inertia, $R1, $R2, $vAverage, $xAverage, $SPbests_average, $Mbest_average);
                    if ($vAverage > $vMaxAverage) {
                        $vAverage = $vMaxAverage;
                    }
                    $vComplex = $this->velocity($inertia, $R1, $R2, $vComplex, $xComplex, $SPbests_complex, $Mbest_complex);
...
for ($iteration = 1; $iteration <= $MAX_ITERATION; $iteration++) {
    ...
    //convert to txt
    $data = array($best_MAE);
    $fp = fopen('hasil_mpso_mbest.txt', 'a');
    fputcsv($fp, $data);
    fclose($fp);
    $absolute_errors = [];
}
