...
class ParticleSwarmOptimizer
{
    ...
    function findSolution($project)
    {
        ...
        for ($iteration = 0; $iteration <= $this->max_iteration - 1; $iteration++) {
            ...
            for ($i = 0; $i <= $this->swarm_size - 1; $i++) {
                if ($iteration === 0) {
                    $R1[$iteration] = $this->chebyshev($iteration, $this->randomZeroToOne());
                    $R2[$iteration] = $this->chebyshev($iteration, $this->randomZeroToOne());
                    //$chaos_value[$iteration] = $this->chebyshev($iteration, $this->randomZeroToOne());

                    $vSimple = $this->randomZeroToOne();
                    $vAverage = $this->randomZeroToOne();
                    $vComplex = $this->randomZeroToOne();

                    $xSimple = $initial_particles[$i]['xSimple'];
                    $xAverage = $initial_particles[$i]['xAverage'];
                    $xComplex = $initial_particles[$i]['xComplex'];

                    $Pbest_simple = $xSimple;
                    $Pbest_average = $xAverage;
                    $Pbest_complex = $xComplex;

                    $Gbest = $this->minimalAE($initial_particles);

                    $vSimple = $this->velocity($inertia, $R1[$iteration], $R2[$iteration], $vSimple, $xSimple, $Pbest_simple, $Gbest['xSimple']);
                    if ($vSimple > $vMaxSimple) {
                        $vSimple = $vMaxSimple;
                    }
                    $vAverage = $this->velocity($inertia, $R1[$iteration], $R2[$iteration], $vAverage, $xAverage, $Pbest_average, $Gbest['xAverage']);
                    if ($vAverage > $vMaxAverage) {
                        $vAverage = $vMaxAverage;
                    }
                    $vComplex = $this->velocity($inertia, $R1[$iteration], $R2[$iteration], $vComplex, $xComplex, $Pbest_complex, $Gbest['xComplex']);
                ...
                if ($iteration > 0) {
                    ...
                    $R1[$iteration] = $this->chebyshev($iteration, $R1[$iteration - 1]);
                    $R2[$iteration] = $this->chebyshev($iteration, $R2[$iteration - 1]);

                    $vSimple = $this->velocity($inertia, $R1[$iteration], $R2[$iteration], $vSimple, $xSimple, $Pbest_simple, $Gbest_simple);
                    if ($vSimple > $vMaxSimple) {
                        $vSimple = $vMaxSimple;
                    }
                    $vAverage = $this->velocity($inertia, $R1[$iteration], $R2[$iteration], $vAverage, $xAverage, $Pbest_average, $Gbest_average);
                    if ($vAverage > $vMaxAverage) {
                        $vAverage = $vMaxAverage;
                    }
                    $vComplex = $this->velocity($inertia, $R1[$iteration], $R2[$iteration], $vComplex, $xComplex, $Pbest_complex, $Gbest_complex);
                    if ($vComplex > $vMaxComplex) {
                        $vComplex = $vMaxComplex;
                    }
                } ## End IF iteration > 0
                ...
                if ($iteration == 0 && ($initial_particles[$i]['ae'] > $particles[$iteration][$i]['ae'] || ($initial_particles[$i]['ae'] == $particles[$iteration][$i]['ae']))) {
                    $Pbest[$iteration][] = $particles[$iteration][$i];
                }
                if ($iteration == 0 && $initial_particles[$i]['ae'] < $particles[$iteration][$i]['ae']) {
                    $Pbest[$iteration][] = $initial_particles[$i];
                }
    
                if ($iteration > 0 && ($particles[$iteration][$i]['ae'] < $Pbest[$iteration - 1][$i]['ae'] || ($particles[$iteration][$i]['ae'] == $Pbest[$iteration - 1][$i]['ae']))) {
                    $Pbest[$iteration][$i] = $particles[$iteration][$i];
                }
                if ($iteration > 0 && ($particles[$iteration][$i]['ae'] > $Pbest[$iteration - 1][$i]['ae'])) {
                    $Pbest[$iteration][$i] = $Pbest[$iteration - 1][$i];
                }
            } ## End of swarm size
...
for ($iteration = 1; $iteration <= $MAX_ITERATION; $iteration++) {
    ...
    $fp = fopen('hasil_chebyshev_convergence.txt', 'a');
    fputcsv($fp, $data);
    fclose($fp);
    $absolute_errors = [];
}
