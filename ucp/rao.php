...
class EvolutionaryRaoAlgorithm
{
    ...
    function findSolution($project)
    {
        for ($generation = 0; $generation <= $this->maximum_generation - 1; $generation++) {
            ...
            if ($generation > 0) {
                for ($i = 0; $i <= $this->particle_size - 1; $i++) {
                    $xSimple = $particles[$generation][$i]['xSimple'] + $r1 * ($best_particles[$generation]['xSimple'] - $worst_particles[$generation]['xSimple']);
                    $xAverage = $particles[$generation][$i]['xAverage'] + $r2 * ($best_particles[$generation]['xAverage'] - $worst_particles[$generation]['xAverage']);
                    $xComplex = $particles[$generation][$i]['xComplex'] + $r3 * ($best_particles[$generation]['xComplex'] - $worst_particles[$generation]['xComplex']);
