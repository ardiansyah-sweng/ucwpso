        ...
        for ($iteration = 0; $iteration <= $this->max_iteration; $iteration++) {
            ...
            if ($iteration > 0) {
                for ($i = 0; $i <= $this->swarm_size - 1; $i++) {
                    ...
                    $vSimple = $this->velocity($inertia, $R1, $R2, $vSimple, $xSimple, $SPbests_simple, $Gbest_simple);
                    if ($vSimple > $vMaxSimple) {
                        $vSimple = $vMaxSimple;
                    }
                    $vAverage = $this->velocity($inertia, $R1, $R2, $vAverage, $xAverage, $SPbests_average, $Gbest_average);
                    if ($vAverage > $vMaxAverage) {
                        $vAverage = $vMaxAverage;
                    }
                    $vComplex = $this->velocity($inertia, $R1, $R2, $vComplex, $xComplex, $SPbests_complex, $Gbest_complex);
...
for ($iteration = 1; $iteration <= $MAX_ITERATION; $iteration++) {
    ...
    //convert to txt
    $data = array($best_MAE);
    $fp = fopen('hasil_mpso_spbest.txt', 'a');
    fputcsv($fp, $data);
    fclose($fp);
    $absolute_errors = [];
}
