...
class PSO
{
    ...
    function Main($dataset, $SWARM_SIZE, $max_iter, $limit_percentage)
    {
        ##Generate Population
...
foreach ($dataset as $key => $val) {
    for ($j = 0; $j <= $run_times - 1; $j++) {
        $result = $mpucwPSO->Main($dataset[$key], $swarm_size, $max_iter, $limit_percentage);
    ...
    echo $final_result['uucw'];
    echo '<br>';
    $arrmae[] = $final_result['ae'];
    $arrUUCW[] = $final_result['uucw'];
...
$sumUAW = array_sum(array_column($dataset, 'uaw'));
$avgUAW = $sumUAW / $numDataset;
$sumTCF = array_sum(array_column($dataset, 'tcf'));
$avgTCF = $sumTCF / $numDataset;
$sumECF = array_sum(array_column($dataset, 'ecf'));
$avgECF = $sumECF / $numDataset;
$sumUUCW = array_sum($arrUUCW);
$avgUUCW = $sumUUCW / $numDataset;
$b0 = 402.295;
$b1 = 3.613;
$b2 = 29.276;
$b3 = -16.816;
...
foreach ($dataset as $key => $val){
    $X1234= $val['uaw'] * $val['tcf'] * $val['ecf'] * $arrUUCW[$key];
    //echo $val['uaw'].' '.$val['tcf'].' '.$val['ecf'].' '.$arrUUCW[$key].' '.$X1234.'<br>';

    $x1 = $val['uaw'] - $avgUAW;
    $x2 = $val['tcf'] - $avgTCF;
    $x3 = $val['ecf'] - $avgECF;
    $x4 = $arrUUCW[$key] - $avgUUCW;
    $X1234 = $x1 * $x2 * $x3 * $x4;
    //echo $x1.' '.$x2.' '.$x3.' '.$x4.' '.$X1234.'<br>';

    $x1_pow = $x1 * $x1;
    $x2_pow = $x2 * $x2;
    $x3_pow = $x3 * $x3;
    $x4_pow = $x4 * $x4;
    //echo $x1_pow.' '.$x2_pow.' '.$x3_pow.' '.$x4_pow.'<br>';

    $ucp = $b0 + ($b1 * $x1) + ($b2 * $x2) + ($b3 * $x3) + ($b4 * $x4) + ($b11 * $x1_pow) + ($b22 * $x2_pow) + ($b33 * $x3_pow) + ($b44 * $x4_pow) + ($b1234 * $X1234);
    $estimated = $ucp * 20;
    $ae = abs($estimated - $val['actualEffort']);
    $mae[] = $ae;
    //echo 'UCP '.$ucp.' Est. '.$estimated.' AE: '.$ae.'<p>';
}

echo '<p>';
$finish = microtime(true);
echo 'Grand MAE PSO: ' . array_sum($arrmae) / $numDataset.' Time: '.($finish-$start);
echo '<br>';
echo 'Grand MAE Hybrid: ' . array_sum($mae) / $numDataset.' Time: '.($finish-$start);
