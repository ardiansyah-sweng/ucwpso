    ...
    function findSolution($project)
    {
        ...
        for ($iteration = 0; $iteration <= $this->max_iteration - 1; $iteration++) {
            ...
            for ($i = 0; $i <= $this->swarm_size - 1; $i++) {
                ...
                if ($iteration === 0) {
                    $Pbest_simple = $initial_particles[$i]['xSimple'];
                    $Pbest_average = $initial_particles[$i]['xAverage'];
                    $Pbest_complex = $initial_particles[$i]['xComplex'];
                    ...
                    $vSimple = $this->velocity($inertia, $R1, $R2, $vSimple, $xSimple, $Pbest_simple, $Gbest['xSimple']);
                    if ($vSimple > $vMaxSimple) {
                        $vSimple = $vMaxSimple;
                    }
                    $vAverage = $this->velocity($inertia, $R1, $R2, $vAverage, $xAverage, $Pbest_average, $Gbest['xAverage']);
                    if ($vAverage > $vMaxAverage) {
                        $vAverage = $vMaxAverage;
                    }
                    $vComplex = $this->velocity($inertia, $R1, $R2, $vComplex, $xComplex, $Pbest_complex, $Gbest['xComplex']);
                ...
                if ($iteration > 0) {
                    ...
                    $vSimple = $this->velocity($inertia, $R1, $R2, $vSimple, $xSimple, $Pbest_simple, $Gbest_simple);
                    if ($vSimple > $vMaxSimple) {
                        $vSimple = $vMaxSimple;
                    }
                    $vAverage = $this->velocity($inertia, $R1, $R2, $vAverage, $xAverage, $Pbest_average, $Gbest_average);
                    if ($vAverage > $vMaxAverage) {
                        $vAverage = $vMaxAverage;
                    }
                    $vComplex = $this->velocity($inertia, $R1, $R2, $vComplex, $xComplex, $Pbest_complex, $Gbest_complex);
                    if ($vComplex > $vMaxComplex) {
                        $vComplex = $vMaxComplex;
                    }
...
for ($iteration = 1; $iteration <= $MAX_ITERATION; $iteration++) {
    for ($trial = 0; $trial <= $trials - 1; $trial++) {
        $optimize = new ParticleSwarmOptimizer($swarm_size, $C1, $C2, $iteration, $max_inertia, $min_inertia, $stopping_value, $dataset, $productivity_factor);
    ...
    //convert to txt
    $data = array($best_MAE);
    $fp = fopen('hasil_psorigin_convergence.txt', 'a');
    fputcsv($fp, $data);
    fclose($fp);
    $absolute_errors = [];
}
