        ...
        for ($iteration = 0; $iteration <= $this->max_iteration - 1; $iteration++) {
            ...
            for ($i = 0; $i <= $this->swarm_size - 1; $i++) {
                ...
                if ($iteration === 0) {
                    ...
                    $vSimple = $this->velocity($inertia[$iteration], $R1[$iteration], $R2[$iteration], $vSimple, $xSimple, $Pbest_simple, $Gbest['xSimple']);
                    if ($vSimple > $vMaxSimple) {
                        $vSimple = $vMaxSimple;
                    }
                    $vAverage = $this->velocity($inertia[$iteration], $R1[$iteration], $R2[$iteration], $vAverage, $xAverage, $Pbest_average, $Gbest['xAverage']);
                    if ($vAverage > $vMaxAverage) {
                        $vAverage = $vMaxAverage;
                    }
                    $vComplex = $this->velocity($inertia[$iteration], $R1[$iteration], $R2[$iteration], $vComplex, $xComplex, $Pbest_complex, $Gbest['xComplex']);
                ...
                if ($iteration > 0) {
                    ...
                    $chaos_value[$iteration] = $this->chebyshev($iteration, $chaos_value[$iteration-1]);
                    $inertia[$iteration] = $chaos_value[$iteration] * $this->min_inertia + (($this->max_inertia - $this->min_inertia) * $iteration / $this->max_iteration);
