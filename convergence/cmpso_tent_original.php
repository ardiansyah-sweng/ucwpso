        ...
        for ($iteration = 0; $iteration <= $this->max_iteration - 1; $iteration++) {
            ...
            for ($i = 0; $i <= $this->swarm_size - 1; $i++) {
                ...
                if ($iteration === 0) {
                    ...
                    $R1[$iteration] = $this->tent($this->randomZeroToOne());
                    $R2[$iteration] = $this->tent($this->randomZeroToOne());
                    $chaos_value[$iteration] = $this->tent($this->randomZeroToOne());
                ...
                if ($iteration > 0) {
                    $R1[$iteration] = $this->tent($R1[$iteration-1]);
                    $R2[$iteration] = $this->tent($R2[$iteration-1]);
                    $chaos_value[$iteration] = $this->tent($chaos_value[$iteration-1]);
                    $inertia[$iteration] = $chaos_value[$iteration] * $this->min_inertia + (($this->max_inertia - $this->min_inertia) * $iteration / $this->max_iteration);
...
foreach ($results[$index] as $key => $best_estimated) {
    ...
    $fp = fopen('hasil_tent_original.txt', 'a');
    fputcsv($fp, $data);
    fclose($fp);
}