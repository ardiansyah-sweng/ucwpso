    ...
    function findSolution($project)
    {
        ...
        for ($iteration = 0; $iteration <= $this->max_iteration - 1; $iteration++) {
            ...
            for ($i = 0; $i <= $this->swarm_size - 1; $i++) {
                ...
                if ($iteration === 0) {
                    $R1[$iteration] = $this->singer($this->randomZeroToOne());
                    $R2[$iteration] = $this->singer($this->randomZeroToOne());
                    $chaos_value[$iteration] = $this->singer($this->randomZeroToOne());
                ...
                if ($iteration > 0) {
                    $R1[$iteration] = $this->singer($R1[$iteration - 1]);
                    $R2[$iteration] = $this->singer($R2[$iteration - 1]);
                    $chaos_value[$iteration] = $this->singer($chaos_value[$iteration - 1]);
                    $inertia[$iteration] = $chaos_value[$iteration] * $this->min_inertia + (($this->max_inertia - $this->min_inertia) * $iteration / $this->max_iteration);
                    ...
                    $xSimple = $particles[$iteration - 1][$i]['xSimple'] + $vSimple;
                    $xAverage = $particles[$iteration - 1][$i]['xAverage'] + $vAverage;
                    $xComplex = $particles[$iteration - 1][$i]['xComplex'] + $vComplex;
...
for ($iteration = 1; $iteration <= $MAX_ITERATION; $iteration++) {
    ...
    //convert to txt
    $data = array($best_MAE);
    $fp = fopen('hasil_singer_convergence.txt', 'a');
    fputcsv($fp, $data);
    fclose($fp);
    $absolute_errors = [];
}
