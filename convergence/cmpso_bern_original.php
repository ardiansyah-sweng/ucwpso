    ...
    function findSolution($project)
    {
        ...
        for ($iteration = 0; $iteration <= $this->max_iteration - 1; $iteration++) {
            ...
            for ($i = 0; $i <= $this->swarm_size - 1; $i++) {
                ...
                if ($iteration === 0) {
                    $R1[$iteration] = $this->bernoulli($this->randomZeroToOne());
                    $R2[$iteration] = $this->bernoulli($this->randomZeroToOne());
                    $chaos_value[$iteration] = $this->bernoulli($this->randomZeroToOne());
                ...
                if ($iteration > 0) {
                    $R1[$iteration] = $this->bernoulli($R1[$iteration - 1]);
                    $R2[$iteration] = $this->bernoulli($R2[$iteration - 1]);
                    $chaos_value[$iteration] = $this->bernoulli($chaos_value[$iteration - 1]);
                    $inertia[$iteration] = $chaos_value[$iteration] * $this->min_inertia + (($this->max_inertia - $this->min_inertia) * $iteration / $this->max_iteration);
...
foreach ($results[$index] as $key => $best_estimated) {
    ...
    //convert to txt
    $data = array($best_estimated['estimatedEffort'], $best_estimated['ae']);
    $fp = fopen('hasil_bern_original.txt', 'a');
    fputcsv($fp, $data);
    fclose($fp);
}
