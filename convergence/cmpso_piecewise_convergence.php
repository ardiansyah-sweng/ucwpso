        ...
        for ($iteration = 0; $iteration <= $this->max_iteration - 1; $iteration++) {
            ...
            for ($i = 0; $i <= $this->swarm_size - 1; $i++) {
                ...
                if ($iteration === 0) {
                    ...
                    $R1[$iteration] = $this->piecewise($this->randomZeroToOne());
                    $R2[$iteration] = $this->piecewise($this->randomZeroToOne());
                    $chaos_value[$iteration] = $this->piecewise($this->randomZeroToOne());
                ...
                if ($iteration > 0) {
                    $R1[$iteration] = $this->piecewise($R1[$iteration-1]);
                    $R2[$iteration] = $this->piecewise($R2[$iteration-1]);
                    $chaos_value[$iteration] = $this->piecewise($chaos_value[$iteration-1]);
                    $inertia[$iteration] = $chaos_value[$iteration] * $this->min_inertia + (($this->max_inertia - $this->min_inertia) * $iteration / $this->max_iteration);
...
for ($iteration = 1; $iteration <= $MAX_ITERATION; $iteration++) {
    ...
    $fp = fopen('hasil_piecewise_convergence.txt', 'a');
    fputcsv($fp, $data);
    fclose($fp);
    $absolute_errors = [];
}