        ...
        for ($iteration = 0; $iteration <= $this->max_iteration - 1; $iteration++) {
            ...
            for ($i = 0; $i <= $this->swarm_size - 1; $i++) {
                ...
                if ($iteration === 0) {
                    ...
                    $R1[$iteration] = $this->sinu($this->randomZeroToOne());
                    $R2[$iteration] = $this->sinu($this->randomZeroToOne());
                    $chaos_value[$iteration] = $this->sinu($this->randomZeroToOne());
                ...
                if ($iteration > 0) {
                    $R1[$iteration] = $this->sinu($R1[$iteration-1]);
                    $R2[$iteration] = $this->sinu($R2[$iteration-1]);
                    $chaos_value[$iteration] = $this->sinu($chaos_value[$iteration-1]);
                    $inertia[$iteration] = $chaos_value[$iteration] * $this->min_inertia + (($this->max_inertia - $this->min_inertia) * $iteration / $this->max_iteration);
...
for ($iteration = 1; $iteration <= $MAX_ITERATION; $iteration++) {
    ...
    $fp = fopen('hasil_sinu_convergence.txt', 'a');
    fputcsv($fp, $data);
    fclose($fp);
    $absolute_errors = [];
}