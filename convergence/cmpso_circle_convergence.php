        ...
        for ($iteration = 0; $iteration <= $this->max_iteration - 1; $iteration++) {
            ...
            for ($i = 0; $i <= $this->swarm_size - 1; $i++) {
                ...
                if ($iteration === 0) {
                    ...
                    $R1[$iteration] = $this->circle($this->randomZeroToOne());
                    $R2[$iteration] = $this->circle($this->randomZeroToOne());
                    $chaos_value[$iteration] = $this->circle($this->randomZeroToOne());
                ...
                if ($iteration > 0) {
                    $R1[$iteration] = $this->circle($R1[$iteration-1]);
                    $R2[$iteration] = $this->circle($R2[$iteration-1]);
                    $chaos_value[$iteration] = $this->circle($chaos_value[$iteration-1]);
                    $inertia[$iteration] = $chaos_value[$iteration] * $this->min_inertia + (($this->max_inertia - $this->min_inertia) * $iteration / $this->max_iteration);
                    ...
                    $xAverage = $xSimple+ $vAverage;
                    $xComplex = $xSimple+ $vComplex;
                    ...
                    if ($particles[$iteration][$i]['ae'] < $Pbest[$iteration-1][$i]['ae']) {
                        $Pbest[$iteration][] = $particles[$iteration][$i];
                    }
...
for ($iteration = 1; $iteration <= $MAX_ITERATION; $iteration++) {
    ...
    $fp = fopen('hasil_circle_convergence.txt', 'a');
    fputcsv($fp, $data);
    fclose($fp);
    $absolute_errors = [];
}