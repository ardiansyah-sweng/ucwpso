...
class PSO
{
    ...
    {
        return (2.3 * POW(2, 0.7)) * sin(pi() * 0.7);
    }
    ...
    function Main($dataset, $SWARM_SIZE, $max_iter)
    {
        ...
        while ($iterasi <= $max_iter - 1) {
            ...
            if ($iterasi == 0) {
                ...
                for ($i = 0; $i <= $SWARM_SIZE - 1; $i++) {
                    //update velocity
                    $vSimple = $w * $this->randomZeroToOne() + $this->C1 * $R1 * ($Pbest[$i]['xSimple'] - $partikelAwal[$i]['xSimple']) + $this->C2 * $R2 * ($GBest['xSimple'] - $partikelAwal[$i]['xSimple']);
                    $vAverage = $w * $this->randomZeroToOne() + $this->C1 * $R1 * ($Pbest[$i]['xAverage'] - $partikelAwal[$i]['xAverage']) + $this->C2 * $R2 * ($GBest['xAverage'] - $partikelAwal[$i]['xAverage']);
                    $vComplex = $w * $this->randomZeroToOne() + $this->C1 * $R1 * ($Pbest[$i]['xComplex'] - $partikelAwal[$i]['xComplex']) + $this->C2 * $R2 * ($GBest['xComplex'] - $partikelAwal[$i]['xComplex']);
            ...
            if ($iterasi != 0) {
                for ($i = 0; $i <= $SWARM_SIZE - 1; $i++) {
                    ...
                    //Update Velocity
                    $vSimple = ($w * $vSimple) + ($this->C1 * $R1) * ($Pbest[$i]['xSimple'] - $xSimple) + ($this->C2 * $R2) * ($GBest['xSimple'] - $xSimple);
                    $vAverage = ($w * $vAverage) + ($this->C1 * $R1) * ($Pbest[$i]['xAverage'] - $xAverage) + ($this->C2 * $R2) * ($GBest['xAverage'] - $xAverage);
                    $vComplex = ($w * $vComplex) + ($this->C1 * $R1) * ($Pbest[$i]['xComplex'] - $xComplex) + ($this->C2 * $R2) * ($GBest['xComplex'] - $xComplex);
