...
class ParticleSwarmOptimizer
{
    ...
    function findSolution($project)
    {
        ...
        for ($iteration = 0; $iteration <= $this->max_iteration; $iteration++) {
            ...
            if ($iteration > 0) {
                $R1[$iteration + 1] = $this->sine($R1[$iteration]);
                $R2[$iteration + 1] = $this->singer($R2[$iteration]);
                for ($i = 0; $i <= $this->swarm_size - 1; $i++) {
                    $Gbest_simple = $Gbest[$iteration]['xSimple'];
                    $Gbest_average = $Gbest[$iteration]['xAverage'];
                    $Gbest_complex = $Gbest[$iteration]['xComplex'];

                    $SPbests_simple = $Pbests[$iteration][$i]['xSimple'];
                    $SPbests_average = $Pbests[$iteration][$i]['xAverage'];
                    $SPbests_complex = $Pbests[$iteration][$i]['xComplex'];
                    ...
                    $vSimple = $this->velocity($inertia, $R1[$iteration], $R2[$iteration], $vSimple, $xSimple, $SPbests_simple, $Gbest_simple);
                    if ($vSimple > $vMaxSimple) {
                        $vSimple = $vMaxSimple;
                    }
                    $vAverage = $this->velocity($inertia, $R1[$iteration], $R2[$iteration], $vAverage, $xAverage, $SPbests_average, $Gbest_average);
                    if ($vAverage > $vMaxAverage) {
                        $vAverage = $vMaxAverage;
                    }
                    $vComplex = $this->velocity($inertia, $R1[$iteration], $R2[$iteration], $vComplex, $xComplex, $SPbests_complex, $Gbest_complex);
...
for ($iteration = 1; $iteration <= $MAX_ITERATION; $iteration++) {
    ...
    //convert to txt
    $data = array($best_MAE);
    $fp = fopen('hasil_chaos_tharwat.txt', 'a');
    fputcsv($fp, $data);
    fclose($fp);
    $absolute_errors = [];
}
