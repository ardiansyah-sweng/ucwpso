...
class MPUCWPSO
{
    ...
    function Main($dataset, $chaotic_map)
    {
        ##Generate Population
        ...
        while ($iterasi <= $this->MAX_ITERATION - 1) {
            if ($iterasi == 0) {
                //Inertia weight
                if ($chaotic_map == 'bernouli') {
                    $random_zeroToOne = $this->randomZeroToOne();
                    $r[$iterasi] = $this->bernouliMap($random_zeroToOne, $iterasi);
                }
            ...
            if ($iterasi != 0) {
                //Inertia weight
                if ($chaotic_map == 'bernouli') {
                    $r[$iterasi] = $this->bernouliMap($r[$iterasi - 1]);
                }
                $w = $r[$iterasi] * $this->INERTIA_MIN + ((($this->INERTIA_MAX - $this->INERTIA_MIN) * $iterasi) / $this->MAX_ITERATION);
...
$max_iter = 40;
$run_times = 100;
$arrChaoticMap = ['bernouli'];

foreach ($arrChaoticMap as $map) {
    foreach ($dataset as $key => $val) {
        for ($j = 0; $j <= $run_times - 1; $j++) {
            $result = $mpucwPSO->Main($dataset[$key], $map);
