...
class MPUCWPSO
{
    ...
    function chaoticLiebovitch($value)
    {
        ...
        if ($value > $P1 && $value <= $P2) {
            return ($P2 - $value) / ($P2 - $P1);
        }
        //P2 < r[iterasi] <= 1
        if ($value > $P2 && $value <= 1) {
            return 1 - ($betha * (1 - $value));
        }
    }
    ...
    function Main($dataset, $max_iter, $swarm_size, $max_counter, $limit_percentage)
    {
        ...
        while ($iterasi <= $max_iter - 1) {
            if ($iterasi == 0) {
                $R1[$iterasi] = $this->chaoticLiebovitch($this->randomZeroToOne());
                $R2[$iterasi] = $this->chaoticLiebovitch($this->randomZeroToOne());

                //Inertia weight
                $r[$iterasi] = $this->chaoticLiebovitch($this->randomZeroToOne());
            ...
            if ($iterasi != 0) {
                //Inertia weight
                $R1[$iterasi] = $this->chaoticLiebovitch($R1[$iterasi-1]);
                $R2[$iterasi] = $this->chaoticLiebovitch($R2[$iterasi-1]);
                $r[$iterasi] = $this->chaoticLiebovitch($r[$iterasi - 1]);
                $w = $r[$iterasi] * $this->INERTIA_MIN + ((($this->INERTIA_MAX - $this->INERTIA_MIN) * $iterasi) / $max_iter);
...
foreach ($results[$bestMAEIndex] as $key => $val) {
    ...
    $fp = fopen('hasil_cmpso_liebovitch_original.txt', 'a');
    fputcsv($fp, $data);
    fclose($fp);
}
