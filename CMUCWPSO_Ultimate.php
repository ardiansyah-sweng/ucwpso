...
class PSO
{
    ...
    #Function chaotic singer map
    function singerMap($last_chaotic)
    {
        $miu = mt_rand(0.9 * 100, 1.08 * 100) / 100;
        return $miu * ((7.86 * $last_chaotic) - (23.31 * POW(2, $last_chaotic)) + (28.75 * POW(3, $last_chaotic)) - (13.3 * POW(4, $last_chaotic)));
    }

    function Main($dataset, $SWARM_SIZE, $ITER)
    {
        ##Generate Population
        ...
        ##Masuk Iterasi
        $iterasi = 0;
        while ($iterasi <= $ITER - 1) {
            //Inertia weight
            $r[$iterasi] = mt_rand(0.9 * 100, 1.08 * 100) / 100;
            $w = $r[$iterasi] * $this->INERTIA_MIN + ((($this->INERTIA_MAX - $this->INERTIA_MIN) * $iterasi) / $ITER);
            ...
            if ($iterasi != 0) {
                //INERTIA WEIGHT
                $r[$iterasi] = $this->singerMap($r[$iterasi - 1]);
                $w = $r[$iterasi] * $this->INERTIA_MIN + ((($this->INERTIA_MAX - $this->INERTIA_MIN) * $iterasi) / $ITER);
...
echo 'Num. iter: ' . $max_iter . ' Size: ' . $swarm_size . ' Time: ' . $duration . ' MAE ' . $avgAE;
echo '<br>';
$arrAE = [];
//}
