...
class MPUCWPSO
{
    ...
    function Main($dataset, $max_iter, $swarm_size, $max_counter, $limit_percentage)
    {
        ...
        while ($iterasi <= $max_iter - 1) {
            ...
            if ($iterasi == 0) {
                $R1[$iterasi] = $this->chaoticSinusoidal($this->randomZeroToOne());
                $R2[$iterasi] = $this->chaoticSinusoidal($this->randomZeroToOne());
                //Inertia weight
                $r[$iterasi] = $this->chaoticSinusoidal($this->randomZeroToOne());
            ...
            if ($iterasi != 0) {
                ...
                $R1[$iterasi] = $this->chaoticSinusoidal($R1[$iterasi-1]);
                $R2[$iterasi] = $this->chaoticSinusoidal($R2[$iterasi-1]);
                //Inertia weight
                $r[$iterasi] = $this->chaoticSinusoidal($r[$iterasi-1]);
                $w = $r[$iterasi] * $this->INERTIA_MIN + ((($this->INERTIA_MAX - $this->INERTIA_MIN) * $iterasi) / $max_iter);
...
foreach ($results[$bestMAEIndex] as $key => $val) {
    ...
    $fp = fopen('hasil_cmpso_sinusoidal_original.txt', 'a');
    fputcsv($fp, $data);
    fclose($fp);
}
