...
class MPUCWPSO
{
    ...
    function chaoticR1R2()
    {
        ...
        if ($random_zeroToOne > 0 && $random_zeroToOne <= $P1) {
            return $alpha * $random_zeroToOne;
        }
        //P1 < r[iterasi] <= P2
        if ($random_zeroToOne > $P1 && $random_zeroToOne <= $P2) {
            return ($P2 - $random_zeroToOne) / ($P2 - $P1);
        }
        //P2 < r[iterasi] <= 1
        if ($random_zeroToOne > $P2 && $random_zeroToOne <= 1) {
            return 1 - ($betha * (1 - $random_zeroToOne));
        }
    }
    ...
    function Main($dataset, $max_iter, $swarm_size, $max_counter, $limit_percentage)
    {
        ...
        while ($iterasi <= $max_iter - 1) {
            ...
            if ($iterasi == 0) {
                ...
                //0 < r[iterasi] <= P1
                if ($random_zeroToOne > 0 && $random_zeroToOne <= $P1) {
                    $r[$iterasi] = $alpha * $random_zeroToOne;
                }
                //P1 < r[iterasi] <= P2
                if ($random_zeroToOne > $P1 && $random_zeroToOne <= $P2) {
                    $r[$iterasi] = ($P2 - $random_zeroToOne) / ($P2 - $P1);
                }
                //P2 < r[iterasi] <= 1
                if ($random_zeroToOne > $P2 && $random_zeroToOne <= 1) {
                    $r[$iterasi] = 1 - ($betha * (1 - $random_zeroToOne));
                }
            ...
            if ($iterasi != 0) {
                ...
                $betha = (1 / ($P2 - 1)) * (($P2 - 1) - ($P1 * ($P2 - $P1)));
                //0 < r[iterasi] <= P1
                if ($r[$iterasi - 1] > 0 && $r[$iterasi - 1] <= $P1) {
                    $r[$iterasi] = $alpha * $r[$iterasi - 1];
                }
                //P1 < r[iterasi] <= P2
                if ($r[$iterasi - 1] > $P1 && $r[$iterasi - 1] <= $P2) {
                    $r[$iterasi] = ($P2 - $r[$iterasi - 1]) / ($P2 - $P1);
                }
                //P2 < r[iterasi] <= 1
                if ($r[$iterasi - 1] > $P2 && $r[$iterasi - 1] <= 1) {
                    $r[$iterasi] = 1 - ($betha * (1 - $r[$iterasi - 1]));
                }
                $w = $r[$iterasi] * $this->INERTIA_MIN + ((($this->INERTIA_MAX - $this->INERTIA_MIN) * $iterasi) / $max_iter);
...
for ($max_iter = 1; $max_iter <= $MAX_ITER; $max_iter++) {
    ...
    $fp = fopen('hasil_mpso_liebovitch.txt', 'a');
    fputcsv($fp, $data);
    fclose($fp);
}